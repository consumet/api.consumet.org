...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  ...
  fastify.get('/', {
    schema: {
      description: 'Get AsuraScans provider info and available routes (Manhwa/Manhua)',
      tags: ['asurascans'],
      response: {
        200: {
          type: 'object',
  ...
  fastify.get('/:query', {
    schema: {
      description: 'Search for manhwa/manhua on AsuraScans',
      tags: ['asurascans'],
      params: {
        type: 'object',
        properties: {
    ...
    try {
      const res = redis
        ? await cache.fetch(
            ...
            async () => await asurascans.search(query, page),
            REDIS_TTL,
          )
  ...
  fastify.get('/info', {
    schema: {
      description: 'Get manhwa/manhua details by ID',
      tags: ['asurascans'],
      querystring: {
        type: 'object',
        properties: {
    ...
    try {
      const res = redis
        ? await cache.fetch(
            redis as Redis,
            `asurascans:info:${id}`,
            async () => await asurascans.fetchMangaInfo(id),
            REDIS_TTL,
          )
    ...
    try {
      const res = redis
        ? await cache.fetch(
            ...
            async () => await asurascans.fetchChapterPages(chapterId),
            REDIS_TTL,
          )
